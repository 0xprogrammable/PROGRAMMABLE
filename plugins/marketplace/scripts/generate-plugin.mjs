...
export const ROOT_CODEX_MANIFEST_PATH = path.join(
  REPOSITORY_ROOT,
  ".codex-plugin",
  "plugin.json"
);
export const ROOT_CLAUDE_MANIFEST_PATH = path.join(
  REPOSITORY_ROOT,
  ".claude-plugin",
  "plugin.json"
);
...
export function materializeDistribution({
  ...
  rootCodexManifestPath = ROOT_CODEX_MANIFEST_PATH,
  rootClaudeManifestPath = ROOT_CLAUDE_MANIFEST_PATH,
  ...
  try {
    ...
    writeAtomically(rootCodexManifestPath, serializeJson(buildCodexManifest(metadata)));
    writeAtomically(rootClaudeManifestPath, serializeJson(buildClaudeManifest(metadata)));
...
export function verifyDistribution({
  ...
  const expectedCodexManifest = expectedManifests.get(".codex-plugin/plugin.json");
  const expectedClaudeManifest = expectedManifests.get(".claude-plugin/plugin.json");
  lstatRegularFile(rootCodexManifestPath, "root Codex plugin.json");
  if (fs.readFileSync(rootCodexManifestPath, "utf8") !== expectedCodexManifest) {
    throw new Error("root Codex plugin.json differs from generated bytes");
  }
  lstatRegularFile(rootClaudeManifestPath, "root Claude plugin.json");
  if (fs.readFileSync(rootClaudeManifestPath, "utf8") !== expectedClaudeManifest) {
    throw new Error("root Claude plugin.json differs from generated bytes");
  }
  ...
  return {
    ...
    codexManifestSha256: sha256(Buffer.from(expectedCodexManifest)),
    claudeManifestSha256: sha256(Buffer.from(expectedClaudeManifest)),
