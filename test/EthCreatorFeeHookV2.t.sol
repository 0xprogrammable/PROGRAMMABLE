...
contract ToggleFeeRecipient {
    ...
    function claimCreatorFeesTo(EthCreatorFeeHookV2 hook, bytes32 poolId, address recipient) external {
        hook.claimCreatorFeesTo(poolId, recipient);
    }

    function claimLauncherFeesTo(EthCreatorFeeHookV2 hook, address recipient) external {
...
contract EthCreatorFeeHookV2Test is Deployers {
    uint16 internal constant TOTAL_SWAP_FEE_BPS = 100;
    uint256 internal constant BASIS_POINTS = 10_000;
    bytes32 internal constant HOOK_FEE_EVENT_SIGNATURE = keccak256("HookFee(bytes32,address,uint128,uint128)");
    bytes32 internal constant URC2_HOOK_SWAP_EVENT_SIGNATURE =
    ...
    function test_feeDisclosureIsExplicitSymmetricAndImmutable() public view {
        (
    ...
    function test_buyEmitsOpenZeppelinHookFeeAndUrc2HookSwap() public {
        uint256 grossInput = 0.1 ether;
        uint256 expectedTotalFee = 0.001 ether;

        vm.recordLogs();
        _swap(hookKey, true, -int256(grossInput), grossInput);

        _assertStandardFeeEvents(vm.getRecordedLogs(), expectedTotalFee);
    }

    function test_sellEmitsOpenZeppelinHookFeeAndUrc2HookSwap() public {
        vm.recordLogs();
        _swap(hookKey, false, -int256(0.01 ether), 0);
        Vm.Log[] memory logs = vm.getRecordedLogs();

        (,,,, uint256 creatorFee) = hook.poolFeeConfig(poolId);
        uint256 expectedTotalFee = creatorFee + hook.launcherFeesAccrued();
        _assertStandardFeeEvents(logs, expectedTotalFee);
    }
    ...
    function test_onlyRecordedRecipientsCanRedirectClaims() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(EthCreatorFeeHookV2.UnauthorizedFeeRedirect.selector, attacker, creatorRecipient)
        );
        hook.claimCreatorFeesTo(poolId, attacker);
        vm.expectRevert(
            abi.encodeWithSelector(EthCreatorFeeHookV2.UnauthorizedFeeRedirect.selector, attacker, launcherTreasury)
        );
    ...
    function test_rejectsFeesOutsideIntegerOneToTenPercentSteps() public {
        for (uint16 feeBps = 0; feeBps <= 1100; feeBps += 50) {
            ...
            vm.expectRevert(abi.encodeWithSelector(EthCreatorFeeHookV2.InvalidTotalSwapFee.selector, feeBps));
    ...
    function test_rejectsRegistrationByAnAddressOtherThanTokenCreator() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(EthCreatorFeeHookV2.InvalidRegistrar.selector, address(this), recordedCreator)
        );
    ...
    function test_factoryRejectsUnminedSalt() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EthCreatorFeeHookFactoryV2.InvalidHookAddress.selector,
                predicted,
                actualFlags,
    ...
    function _deployHook() private returns (EthCreatorFeeHookV2 deployed, bytes32 salt) {
        (, salt) = HookMiner.find(
            address(factory),
            factory.REQUIRED_HOOK_FLAGS(),
    ...
    function _assertStandardFeeEvents(Vm.Log[] memory logs, uint256 expectedTotalFee) private view {
        uint256 hookFeeCount;
        uint256 hookSwapCount;
        ...
        for (uint256 index = 0; index < logs.length; index++) {
            ...
            if (entry.topics[0] == HOOK_FEE_EVENT_SIGNATURE) {
                hookFeeCount++;
                assertEq(entry.topics[1], poolId);
                assertEq(address(uint160(uint256(entry.topics[2]))), address(swapRouter));
                (uint128 feeAmount0, uint128 feeAmount1) = abi.decode(entry.data, (uint128, uint128));
                assertEq(feeAmount0, expectedTotalFee);
                assertEq(feeAmount1, 0);
            } else if (entry.topics[0] == URC2_HOOK_SWAP_EVENT_SIGNATURE) {
                hookSwapCount++;
                assertEq(entry.topics[1], poolId);
                assertEq(address(uint160(uint256(entry.topics[2]))), address(swapRouter));
                (int128 amount0, int128 amount1, uint24 swapFee) = abi.decode(entry.data, (int128, int128, uint24));
                assertEq(amount0, -int256(expectedTotalFee));
                assertEq(amount1, 0);
                assertEq(swapFee, 10_000);
            }
        }

        assertEq(hookFeeCount, 1, "one HookFee event per charged swap");
        assertEq(hookSwapCount, 1, "one URC-2 HookSwap event per charged swap");
    }
}
