...
import { TollHookV1 } from "./TollHookV1.sol";
import { TollPositionForwarderFactory } from "./TollPositionForwarderFactory.sol";
...
contract TollLaunchV1 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    TollHookV1 public immutable tollHook;
    ...
    TollPositionForwarderFactory public immutable positionForwarderFactory;
    ...
    constructor(
        ...
        if (
            address(tollHook_.poolManager()) != address(poolManager_) || tollHook_.LP_FEE_PIPS() != LP_FEE_PIPS
                || tollHook_.TICK_SPACING() != TICK_SPACING
        ) {
            revert InvalidSharedHook(address(poolManager_), tollHook_.LP_FEE_PIPS(), tollHook_.TICK_SPACING());
        }
        address configuredVaultFactory = address(tollHook_.rewardVaultFactory());
    ...
    function predictRewardVault(
        ...
        return rewardVaultFactory.predict(
            _rewardVaultSalt(token, deployer), IClassicFeeHookV3(address(tollHook)), poolId, beneficiaries, sharesBps
        );
    }
        ...
        returns (LaunchResult memory result)
    {
        ...
        bytes32 registeredPoolId =
            tollHook.registerPool(key, result.rewardVault, parameters.fees);
    ...
    function _emitTokenLaunched(
        ...
        emit TollTokenLaunched(
            ...
            parameters.fees.buyFeeBps,
            parameters.fees.sniperSellFeeBps,
            parameters.fees.diamondSellFeeBps,
    ...
    function _deployOrReusePositionRecipient(address token, address deployer) private returns (address recipient) {
        ...
        TollPositionForwarder forwarder = TollPositionForwarder(payable(recipient));
        if (
