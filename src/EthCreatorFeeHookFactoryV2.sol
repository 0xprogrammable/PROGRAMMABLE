...
import { EthCreatorFeeHookV2 } from "./EthCreatorFeeHookV2.sol";

/// @title EthCreatorFeeHookFactoryV2
/// @notice Deterministically deploys the URC-2-compatible shared ETH creator-fee hook at a valid v4 hook address.
contract EthCreatorFeeHookFactoryV2 {
    ...
    function deploy(bytes32 salt, IPoolManager poolManager, address launcherFeeRecipient)
        external
        returns (EthCreatorFeeHookV2 hook)
    {
        ...
        hook = EthCreatorFeeHookV2(deployed);
        ...
        emit EthCreatorFeeHookDeployed(deployed, address(poolManager), launcherFeeRecipient, salt, configurationHash);
    }
    ...
    function initCode(IPoolManager poolManager, address launcherFeeRecipient) public pure returns (bytes memory) {
        ...
        return abi.encodePacked(type(EthCreatorFeeHookV2).creationCode, abi.encode(poolManager, launcherFeeRecipient));
    }
