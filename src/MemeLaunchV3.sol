...
/// @title MemeLaunchV3
/// @notice Router-only future Classic launcher with immutable directional fees and beneficiary-owned rewards.
/// @dev Preserves V2's UERC20, pool, locked one-sided position and initial-buy mechanics while binding all creator
///      semantics to the explicit launch wallet rather than to the router caller.
contract MemeLaunchV3 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    // The release ABI deliberately exposes the pinned authority using its canonical all-caps name.
    ...
    event MemeLiquidityConfiguredV3(
        address indexed token,
        uint256 totalSupply,
        uint256 tokenLiquidityAmount,
        uint256 lockedTokenDust,
    ...
    event MemeCreatorInitialBuyCustodyV3(
        address indexed launchWallet,
        address indexed token,
    ...
    constructor(
        ...
        // The require-contract check above proves both nonzero and live code; Slither does not follow that helper.
    ...
    function predictRewardVault(
        ...
        return rewardVaultFactory.predict(
            _rewardVaultSalt(token, launchWallet), IClassicFeeHookV3(address(feeHook)), poolId, beneficiaries, sharesBps
        );
    }

    /// @notice Creates, registers, initializes and permanently positions a Classic launch for `launchWallet`.
    /// @dev Only the immutable router can call this future-only entry point; `launchWallet` is the creator authority.
        ...
        returns (LaunchResult memory result)
    {
        ...
        PoolKey memory key;
        (result, key) = _createAndInitializeLaunch(launchWallet, parameters, result);
        Position memory position = _configureLaunchLiquidity(key, result);
        // ReentrancyGuardTransient protects every external call and the final state recording in this launch.
        // slither-disable-next-line reentrancy-benign
        return _completeLaunch(launchWallet, parameters, key, position, result);
    }

    function _createAndInitializeLaunch(
        address launchWallet,
        LaunchParameters calldata parameters,
        LaunchResult memory result
    ) private returns (LaunchResult memory, PoolKey memory key) {
    ...
    function _configureLaunchLiquidity(PoolKey memory key, LaunchResult memory result)
        private
        returns (Position memory position)
    {
        ...
        Plan memory plan;
        uint256 lockedTokenDust;
        (plan, position, lockedTokenDust) = _buildOneSidedPlan(key, result.positionRecipient, initialSqrtPriceX96);
    ...
    function _recordLaunch(
        ...
        bytes32 custodyConfigurationHash = _initialBuyCustodyConfigurationHash(parameters, result, launchWallet);
        bytes32 infrastructureHash =
            _infrastructureHash(result, launchWallet, rewardConfigurationHash, custodyConfigurationHash);
        ...
        _emitLaunchEvents(
            parameters, result, position, launchWallet, rewardConfigurationHash, custodyConfigurationHash, launchHash
        );
    }
    ...
    function _emitLaunchEvents(
        ...
        _emitTokenLaunched(parameters, result, launchWallet, rewardConfigurationHash, launchHash);
        emit MemeLiquidityConfiguredV3(
            result.token,
            TOKEN_SUPPLY,
        ...
        emit MemeCreatorInitialBuyCustodyV3(
            launchWallet,
            result.token,
    ...
    function _deployOrReusePositionRecipient(address token, address launchWallet) private returns (address recipient) {
        recipient = positionForwarderFactory.predict(_positionSalt(token, launchWallet), launchWallet);
        if (recipient.code.length == 0) {
            return address(positionForwarderFactory.deploy(_positionSalt(token, launchWallet), launchWallet));
        }
    ...
    function _deployOrReuseInitialBuyCustody(
        ...
        return address(
            initialBuyVestingWalletFactory.deployOrGet(
                _initialBuyCustodySalt(token, launchWallet),
                IERC20(token),
                launchWallet,
    ...
    function _deployOrReuseRewardVault(
        ...
        return address(
            rewardVaultFactory.deployOrGet(
                _rewardVaultSalt(token, launchWallet),
    ...
    function _effectiveGraffiti(address launchWallet, bytes32 creatorSalt) private pure returns (bytes32) {
        return keccak256(abi.encode(launchWallet, creatorSalt));
    }

    function _positionSalt(address token, address launchWallet) private pure returns (bytes32) {
        return keccak256(abi.encode("launcher.meme-position.v1", token, launchWallet));
    }

    function _rewardVaultSalt(address token, address launchWallet) private pure returns (bytes32) {
        return keccak256(abi.encode("programmable.classic-reward-vault.v1", token, launchWallet));
    }

    // Slither cannot build IR for the caller and therefore misses this use.
    // slither-disable-next-line dead-code
    function _initialBuyCustodySalt(address token, address launchWallet) private pure returns (bytes32) {
        return keccak256(abi.encode("programmable.classic-initial-buy-custody.v1", token, launchWallet));
    }
