...
**Model id:** `hookemon`
**Authoritative preflight state:** design changes required until the owner decision under “Open decisions” is closed.
...
3. The deployment Safe creates initial liquidity after allocation and LP custody are approved.
...
| Operator | Durable checkpoint state machine with bounded Collector/CCTP/reward ports. Live signing remains disabled. |
...
| CycleVault admin Safe | A new Hookemon-only Safe; exact address is bound and reviewed before deployment. | Propose and execute capped bridge/reward configuration and administer project-cycle settings. | Yes | 48 hours for configuration | Cannot redirect the Programmable liability or alter token transfers; incorrect settings can delay project cycles. |
| Guardian | A new Hookemon-only address; exact address is bound and reviewed before deployment. | Pause new bridge cycles and pause reward funding/settlement. | Rotatable under the reviewed Safe policy | Immediate pause only | Can delay automation but cannot sweep or forfeit accrued claims and cannot block LP exits. |
| Ethereum/Solana policy wallets | New policy-constrained wallets; exact addresses are bound and reviewed before deployment. | Submit bounded CCTP/Collector transactions, publish reviewed roots and sponsor settlement gas. | Rotatable under the reviewed Safe and incident policy | Policy-controlled rotation | Failure delays cycles; contract caps, public proofs and immutable fee liabilities limit redirection. |

## Provenance

- **Builder-stated:** project name Hookemon; memecoin/collectible concept; 3% total; automatic Collector packs; automatic top-holder distribution; 500 USDC-equivalent bootstrap intent.
...
1. **Launch economics** — Which exact mainnet token allocation, launch terms, initial HOOKEMON/USDC liquidity, LP position owner/lock and residual treasury treatment will the owner approve?

## Deployment and release prerequisites

- Bind independently reviewed Hookemon-only Safe, guardian, Ethereum policy-wallet and Solana policy-wallet addresses before any deployment rehearsal.
