...
contract VaultMockHook is IClassicFeeHookV3 {
    ...
    contract FeeSplitVaultV1Test is Test {
        bytes32 internal constant POOL_ID = keccak256("classic-v3-pool");

        FeeSplitVaultFactoryV1 internal factory;
        VaultMockPoolManager internal manager;
        VaultMockHook internal hook;

        function setUp() public {
            factory = new FeeSplitVaultFactoryV1();
        ...
        function test_factoryDeploysAtPredictedAddressAndCommitsConfiguration() public {
            ...
            FeeSplitVaultV1 vault = factory.deploy(salt, hook, POOL_ID, beneficiaries, shares);
        ...
        function test_supportsEightImmutableBeneficiaries() public {
            address[] memory beneficiaries = _beneficiaries(8);
            ...
            FeeSplitVaultV1 vault = factory.deploy(bytes32("eight"), hook, POOL_ID, beneficiaries, shares);
            assertEq(vault.beneficiaryCount(), 8);
            for (uint256 index; index < 8; index++) {
                assertEq(vault.beneficiaryAt(index), beneficiaries[index]);
                assertEq(vault.payoutAddressOf(beneficiaries[index]), beneficiaries[index]);
            }
        }

        function test_rejectsZeroAndMoreThanEightBeneficiaries() public {
            address[] memory none = new address[](0);
            uint16[] memory noShares = new uint16[](0);
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidBeneficiaryCount.selector, 0));
            factory.deploy(bytes32("none"), hook, POOL_ID, none, noShares);

            address[] memory nine = _beneficiaries(9);
            uint16[] memory shares = new uint16[](9);
            for (uint256 index; index < 8; index++) {
                shares[index] = 1000;
            }
            shares[8] = 2000;
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidBeneficiaryCount.selector, 9));
            factory.deploy(bytes32("nine"), hook, POOL_ID, nine, shares);
        }
        ...
        function test_rejectsZeroDuplicateAndZeroShareBeneficiaries() public {
            ...
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidBeneficiary.selector, address(0)));
            ...
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.DuplicateBeneficiary.selector, beneficiaries[0]));
            factory.deploy(bytes32("duplicate"), hook, POOL_ID, beneficiaries, shares);

            beneficiaries = _beneficiaries(2);
            shares[0] = 0;
            shares[1] = 10_000;
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidShare.selector, beneficiaries[0], 0));
        ...
        function test_rejectsShareTotalOtherThanTenThousand() public {
            ...
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidShareTotal.selector, 9000));
        ...
        function test_acceptsSmartAndCounterfactualWalletBeneficiaries() public {
            ...
            FeeSplitVaultV1 vault = factory.deploy(
        ...
        function test_onlyBeneficiaryCanChangeItsOwnPayoutAddress() public {
            address beneficiary = makeAddr("beneficiary");
            address attacker = makeAddr("attacker");
            FeeSplitVaultV1 vault =
                factory.deploy(bytes32("authority"), hook, POOL_ID, _addresses1(beneficiary), _shares1(10_000));
            vm.prank(attacker);
            ...
            vm.expectRevert(abi.encodeWithSelector(FeeSplitVaultV1.InvalidPayoutAddress.selector, address(0)));
            vault.setPayoutAddress(address(0));
        }
