...
contract EthCreatorFeeHookHandler {
    using SafeCast for uint256;

    PoolSwapTest internal immutable router;
    EthCreatorFeeHookV1 internal immutable hook;
    ...
    constructor(PoolSwapTest router_, EthCreatorFeeHookV1 hook_, IERC20 token_, PoolKey memory key_) payable {
        router = router_;
        hook = hook_;
        token = token_;
        key = key_;
...
contract EthCreatorFeeHookInvariantTest is Deployers {
    ...
    function setUp() public {
        ...
        handler = new EthCreatorFeeHookHandler{ value: 10_000 ether }(swapRouter, hook, IERC20(address(token)), hookKey);
        assertTrue(token.transfer(address(handler), 1e30));

        bytes4[] memory selectors = new bytes4[](6);
        selectors[0] = EthCreatorFeeHookHandler.buyExactInput.selector;
        selectors[1] = EthCreatorFeeHookHandler.sellExactInput.selector;
        selectors[2] = EthCreatorFeeHookHandler.buyExactOutput.selector;
        selectors[3] = EthCreatorFeeHookHandler.sellExactOutput.selector;
        selectors[4] = EthCreatorFeeHookHandler.claimCreator.selector;
        selectors[5] = EthCreatorFeeHookHandler.claimLauncher.selector;
