...
contract LedgerHookFixture is IUnlockCallback {
    ...
    function registerPool(bytes32 pool, address[] memory creators, uint16[] memory shares, bytes32[] memory families)
        external
    {
...
contract ClassicModuleFeeLedgerV1Test is Test {
    ...
    function test_registrationSupportsTenCreatorSlotsAndArbitraryBps() public {
        ...
        assertEq(
            ...
            keccak256(
                abi.encode(block.chainid, address(ledger), address(hook), FIRST_POOL, creators, shares, _families(8))
            )
        );
    }
    ...
    function _assertSameContributions(bytes32 first, bytes32 second, uint256 modules) private view {
        ...
        for (uint256 index; index < 3; index++) {
            assertEq(
                ledger.contributionByPool(first, _creator(index)), ledger.contributionByPool(second, _creator(index))
            );
        }
    }
