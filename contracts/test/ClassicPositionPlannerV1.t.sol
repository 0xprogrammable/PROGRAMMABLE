...
contract ClassicPositionPlannerV1Test is Test {
    ...
    uint256 private constant EXPECTED_DEEP30_RATIO_MPS = 1_298_604_130;
    ...
    function test_deep30PresetIsOnePositionAndApproximately1298604PercentOfLegacyLiquidity() public view {
        (, Position memory standard,) = planner.buildOneSidedPlan(key, recipient, planner.STANDARD_PRESET());
        (Plan memory plan, Position memory deep30, uint256 dust) =
            planner.buildOneSidedPlan(key, recipient, planner.DEEP30_PRESET());

        uint256 ratioMps = deep30.liquidity * MPS / standard.liquidity;
        assertApproxEqAbs(ratioMps, EXPECTED_DEEP30_RATIO_MPS, 2);
        assertEq(deep30.tickLower, planner.DEEP30_TICK_LOWER());
        assertEq(deep30.tickUpper, planner.INITIAL_TICK());
        assertEq(deep30.amount0, 0);
        assertEq(deep30.amount1 + dust, planner.TOKEN_SUPPLY());
