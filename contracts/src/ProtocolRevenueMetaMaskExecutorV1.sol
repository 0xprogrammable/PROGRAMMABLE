...
interface IProtocolRevenueReceiverV1 is IERC165 {
...
contract ProtocolRevenueMetaMaskExecutorV1 is IProtocolRevenueReceiverV1, ReentrancyGuardTransient {
    ...
    uint64 public constant ETHEREUM_MAINNET_CHAIN_SELECTOR = 5_009_297_550_715_157_269;
    uint64 public constant MAX_REPORT_AGE = 6 hours;
    uint64 public constant MAX_REPORT_FUTURE_SKEW = 5 minutes;
    /// @dev CRE publishes the first ten ASCII hex characters of SHA-256(workflow name), not the raw name.
    ///      SHA-256("revenue-v1") starts with `2ffd9244e8`.
    ...
    bytes32 public constant CRE_FORWARDER_CODE_HASH =
    ...
    uint64 public lastAcceptedScheduledAt;
    ...
    error FutureReport(uint64 scheduledAt, uint256 latestAllowed);
    error InvalidDelegation();
    error InvalidMetadataLength(uint256 actual);
    error InvalidReportLength(uint256 actual);
    ...
    error ReportReplay(uint64 scheduledAt, uint64 lastAcceptedScheduledAt);
    error StaleReport(uint64 scheduledAt, uint256 oldestAllowed);
    error UnexpectedChainSelector(uint64 actual, uint64 expected);
    ...
    constructor(IProtocolRevenueRouterTargetV1 router_, IProtocolRevenueExecutionEnforcerTargetV1 enforcer_) {
        ...
        if (
            block.chainid != 1 || routerAddress.code.length == 0 || enforcerAddress.code.length == 0
                ...
                || enforcer_.REVENUE_AUTHORITY() != REVENUE_AUTHORITY
    ...
    function onReport(bytes calldata metadata, bytes calldata report) external override nonReentrant {
        if (msg.sender != CRE_FORWARDER) revert OnlyCREForwarder(msg.sender);
        _assertBindings();
        if (metadata.length != 62) revert InvalidMetadataLength(metadata.length);
        (bytes10 workflowName, address workflowOwner) = _workflowIdentity(metadata);
        if (workflowName != CRE_WORKFLOW_NAME || workflowOwner != CRE_WORKFLOW_OWNER) {
            revert UnexpectedWorkflow(workflowName, workflowOwner);
        }
        if (report.length != 96) revert InvalidReportLength(report.length);

        (uint64 chainSelector, uint64 scheduledAt, int24 referenceTick) = abi.decode(report, (uint64, uint64, int24));
        if (chainSelector != ETHEREUM_MAINNET_CHAIN_SELECTOR) {
            revert UnexpectedChainSelector(chainSelector, ETHEREUM_MAINNET_CHAIN_SELECTOR);
        }
        if (scheduledAt <= lastAcceptedScheduledAt) revert ReportReplay(scheduledAt, lastAcceptedScheduledAt);
        uint256 latestAllowed = block.timestamp + MAX_REPORT_FUTURE_SKEW;
        if (scheduledAt > latestAllowed) revert FutureReport(scheduledAt, latestAllowed);
        // Six-hour report freshness is insensitive to a validator's seconds of timestamp discretion.
        // forge-lint: disable-next-line(block-timestamp)
        uint256 oldestAllowed = block.timestamp > MAX_REPORT_AGE ? block.timestamp - MAX_REPORT_AGE : 0;
        if (scheduledAt < oldestAllowed) revert StaleReport(scheduledAt, oldestAllowed);

        lastAcceptedScheduledAt = scheduledAt;
        _executeCycle(scheduledAt, referenceTick);
    }
    ...
    function supportsInterface(bytes4 interfaceId) external pure override returns (bool) {
        return interfaceId == type(IProtocolRevenueReceiverV1).interfaceId || interfaceId == type(IERC165).interfaceId;
    }
    ...
    function _assertBindings() private view {
        ...
        _assertCodeHash(CRE_FORWARDER, CRE_FORWARDER_CODE_HASH);
    ...
    function _workflowIdentity(bytes calldata metadata) private pure returns (bytes10 name, address owner) {
