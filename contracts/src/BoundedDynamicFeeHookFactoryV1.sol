...
import { BoundedDynamicFeeHookV1 } from "./BoundedDynamicFeeHookV1.sol";

/// @title BoundedDynamicFeeHookFactoryV1
/// @notice Permissionless deterministic deployer and provenance registry for bounded dynamic-fee hooks.
contract BoundedDynamicFeeHookFactoryV1 {
    uint160 public constant ALL_HOOK_MASK = (1 << 14) - 1;
    uint160 public constant REQUIRED_HOOK_FLAGS = (1 << 13) | (1 << 12) | (1 << 7) | (1 << 6) | (1 << 2);
    ...
    event BoundedDynamicFeeHookDeployed(
    ...
    /// @notice Deploys a hook after its salt has been mined for the exact callback mask.
    function deploy(
        bytes32 salt,
        ...
        Currency currency0,
        Currency currency1
    ) external returns (BoundedDynamicFeeHookV1 hook) {
        ...
        hook = new BoundedDynamicFeeHookV1{ salt: salt }(poolManager, authorized, feeRecipient, currency0, currency1);
        ...
        emit BoundedDynamicFeeHookDeployed(
            address(hook),
            configurationHash,
            salt,
            address(poolManager),
    ...
    function initCodeHash(
        ...
        bytes memory constructorArguments = abi.encode(poolManager, authorized, feeRecipient, currency0, currency1);
        // Slither misclassifies the concatenated creation-code expression as a numeric literal.
        // slither-disable-next-line too-many-digits
        return keccak256(bytes.concat(type(BoundedDynamicFeeHookV1).creationCode, constructorArguments));
    }
