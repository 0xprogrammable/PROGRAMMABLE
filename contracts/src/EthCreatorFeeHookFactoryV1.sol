...
/// @title EthCreatorFeeHookFactoryV1
/// @notice Deterministically deploys the shared ETH-only creator fee hook at a valid Uniswap v4 hook address.
contract EthCreatorFeeHookFactoryV1 {
    ...
    function deploy(bytes32 salt, IPoolManager poolManager, address launcherFeeRecipient)
        external
        returns (EthCreatorFeeHookV1 hook)
    {
        ...
        hook = EthCreatorFeeHookV1(deployed);
    ...
    function initCode(IPoolManager poolManager, address launcherFeeRecipient) public pure returns (bytes memory) {
        ...
        return abi.encodePacked(type(EthCreatorFeeHookV1).creationCode, abi.encode(poolManager, launcherFeeRecipient));
    }
