...
interface IFullRangeInitializationAuthorityV1 {
    function initializationCommitment(address vault) external view returns (bytes32);
}

/// @title LiquidityGrowthFullRangeVaultV1
...
///      is capped to 0.25% of native virtual depth from Programmable's locked active positions and uses a fixed
///      30-minute cooldown. There is no liquidity-removal, rescue, admin or upgrade path.
contract LiquidityGrowthFullRangeVaultV1 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    function initialize(
        ...
        bytes32 expected = IFullRangeInitializationAuthorityV1(FACTORY).initializationCommitment(address(this));
        ...
        )
    {
        ...
        (trustedNativeDepth, depthCapNative) = trustedDepthAndCap();
        // A bounded keeper cooldown intentionally uses timestamp granularity; validator skew cannot bypass 30 minutes.
    ...
    function _compoundOneChunk(address caller) private returns (CompoundResult memory result) {
        ...
        // The quote is used only as an exact-pool, full-history and spot-deviation circuit breaker. Full-Range V1
        ...
        (, uint256 depthCap) = trustedDepthAndCap();
        ...
        returns (uint256 count)
    {
        ...
        for (uint256 index; index < count; index++) {
            ...
            uint16 share = sharesBps[index];
            _beneficiaries.push(beneficiary);
            shareBpsOf[beneficiary] = share;
    ...
    function _deployOrReuseUpstreamVault(FeeSplitVaultFactoryV1 factory) private returns (FeeSplitVaultV1 vault) {
        ...
        bytes32 salt =
            keccak256(abi.encode("programmable.liquidity-growth.full-range.upstream.v1", address(this), poolId));
    ...
    function _rewardEntitlement(address beneficiary, uint256 totalReceived) private view returns (uint256 amount) {
        ...
        for (uint256 index; index + 1 < count; index++) {
            allocatedBeforeRemainder += FullMath.mulDiv(totalReceived, shareBpsOf[_beneficiaries[index]], BASIS_POINTS);
        }
