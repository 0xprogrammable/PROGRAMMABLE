...
/// @title StockPairedPositionPlannerV3
/// @notice Stateless adapter around Uniswap's official PositionPlanner with a launch-specific absolute tick.
contract StockPairedPositionPlannerV3 {
    uint256 public constant TOKEN_SUPPLY = 1_000_000_000 ether;
    int24 public constant TICK_SPACING = 200;
    uint24 private constant POSITION_WEIGHT = 10_000_000;

    error InvalidInitialAbsoluteTick(int24 tick);
    ...
    function buildOneSidedPlan(
        ...
        int24 initialAbsoluteTick
    ) external pure returns (Plan memory plan, Position memory position, uint256 lockedTokenDust) {
        _validateInitialAbsoluteTick(initialAbsoluteTick);
        int24 initialTick = quoteIsCurrency0 ? initialAbsoluteTick : -initialAbsoluteTick;
    ...
    function _validateInitialAbsoluteTick(int24 tick) private pure {
        if (
            tick <= 0 || tick % TICK_SPACING != 0 || tick >= TickMath.maxUsableTick(TICK_SPACING)
                || -tick <= TickMath.minUsableTick(TICK_SPACING)
        ) {
            revert InvalidInitialAbsoluteTick(tick);
        }
    }
}
