...
/// @title LiquidityGrowthFullRangeAutomationV1
...
///      vaults. An offchain keeper may poll every five minutes; the vault still owns the 30-minute compound cooldown.
contract LiquidityGrowthFullRangeAutomationV1 is ReentrancyGuardTransient {
    ...
    LiquidityGrowthFullRangeVaultFactoryV1 public immutable vaultFactory;
    ...
    constructor(LiquidityGrowthFullRangeVaultFactoryV1 vaultFactory_, address launcher_) {
    ...
    function assessVault(address vaultAddress) external view returns (Action action) {
        ...
        (LiquidityGrowthFullRangeVaultV1.WorkAction vaultAction,,,,,) = vault.workState();
        if (vaultAction == LiquidityGrowthFullRangeVaultV1.WorkAction.Process) return Action.ProcessFees;
        if (vaultAction == LiquidityGrowthFullRangeVaultV1.WorkAction.Compound) return Action.CompoundPending;
        return Action.None;
    }
    ...
    function _performOne(address vaultAddress) private returns (bool succeeded, Action action) {
        ...
        if (action == Action.GrowOracle) {
            ...
            payload = abi.encodeCall(LiquidityGrowthFullRangeVaultV1.process, ());
        } else {
            payload = abi.encodeCall(LiquidityGrowthFullRangeVaultV1.compoundPending, ());
        }
    ...
    function _registerVault(address vaultAddress) private returns (bool newlyRegistered) {
        LiquidityGrowthFullRangeVaultV1 vault = _requireFactoryVault(vaultAddress);
    ...
    function _requireFactoryVault(address vaultAddress) private view returns (LiquidityGrowthFullRangeVaultV1 vault) {
        ...
        vault = LiquidityGrowthFullRangeVaultV1(payable(vaultAddress));
