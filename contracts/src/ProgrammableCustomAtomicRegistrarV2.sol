...
/// @dev This is intentionally independent from the immutable Generation 1 registrar implementation. Generation 2
///      snapshots any ETH that was already forced into the registrar and requires the launch to preserve that exact
///      balance. A third party therefore cannot permanently deny service by force-sending ETH before a launch.
contract ProgrammableCustomAtomicRegistrarV2 is ReentrancyGuard {
    ...
    {
        // msg.value is already included in address(this).balance at function entry. Subtracting it records only ETH
        // that predates this launch, including ETH delivered through SELFDESTRUCT without invoking this contract.
        ...
        if (address(this).balance != preexistingBalance) {
            revert ResidualValue(address(this).balance);
        }
