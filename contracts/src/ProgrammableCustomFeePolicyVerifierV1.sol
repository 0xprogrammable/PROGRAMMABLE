...
/// @notice Internal, deterministic policy verifier shared by the registry and its public verifier facade.
library ProgrammableCustomFeePolicyVerifierLibV1 {
    address internal constant PROGRAMMABLE_FEE_RECIPIENT = 0x4957f49620AFf3Adbbe8195a4f633E49cc93376c;
    bytes32 internal constant AEON_PROVIDER_ID = keccak256("aeon");
    bytes32 internal constant PARTNER_STATUS_ACTIVE_ID = keccak256("programmable.partner-status.active.v1");
    bytes32 internal constant FEE_POLICY_DOMAIN = keccak256("programmable.custom-fee-policy.v1");
    ...
    function _validatePartner(IProgrammableCustomRegistryV1.FeePolicyV1 calldata policy) private pure {
        if (
            policy.providerId != AEON_PROVIDER_ID || policy.partnerStatusId != PARTNER_STATUS_ACTIVE_ID
...
/// @title ProgrammableCustomFeePolicyVerifierV1
/// @notice Stateless public facade for the exact native and partner fee-policy checks used by the registry.
contract ProgrammableCustomFeePolicyVerifierV1 {
    address public constant PROGRAMMABLE_FEE_RECIPIENT =
        ProgrammableCustomFeePolicyVerifierLibV1.PROGRAMMABLE_FEE_RECIPIENT;
    bytes32 public constant PARTNER_STATUS_ACTIVE_ID =
        ProgrammableCustomFeePolicyVerifierLibV1.PARTNER_STATUS_ACTIVE_ID;
    bytes32 public constant AEON_PROVIDER_ID = ProgrammableCustomFeePolicyVerifierLibV1.AEON_PROVIDER_ID;

    function verify(IProgrammableCustomRegistryV1.FeePolicyV1 calldata policy) external pure returns (bytes32) {
        return ProgrammableCustomFeePolicyVerifierLibV1.validateAndHash(policy);
    }
}
