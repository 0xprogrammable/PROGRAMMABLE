...
import { StockPairedLaunchV1 } from "./StockPairedLaunchV1.sol";
...
/// @title StockPairedEthLaunchCoordinatorV1
/// @notice Converts one ETH Initial Buy into a reviewed stock quote asset and atomically launches through
///         StockPairedLaunchV1. The coordinator has no owner, upgrade path or retained user balances.
contract StockPairedEthLaunchCoordinatorV1 is ReentrancyGuardTransient {
    using SafeERC20 for IERC20;

    uint24 public constant WETH_USDC_FEE = 500;

    StockPairedLaunchV1 public immutable launcher;
    IUniswapV3SwapRouterLike public immutable v3SwapRouter;
    ...
    struct EthLaunchParameters {
        ...
        uint256 deadline;
        StockPairedLaunchV1.LaunchParameters launch;
    }
    ...
    constructor(
        StockPairedLaunchV1 launcher_,
        IUniswapV3SwapRouterLike v3SwapRouter_,
    ...
    function effectiveCreatorSalt(address creator, bytes32 creatorSalt) public pure returns (bytes32) {
        return keccak256(abi.encode("programmable.stock-paired-eth-launch.v1", creator, creatorSalt));
    }
        ...
        returns (StockPairedLaunchV1.LaunchResult memory result)
    {
        ...
        StockPairedLaunchV1.LaunchParameters memory launchParameters = parameters.launch;
    ...
    function _emitLaunch(StockPairedLaunchV1.LaunchResult memory result, address creator, uint256 ethAmount) private {
