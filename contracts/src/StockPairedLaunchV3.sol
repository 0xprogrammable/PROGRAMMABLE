...
contract StockPairedLaunchV3 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    uint256 public constant CONFIGURED_QUOTE_ASSET_COUNT = 6;
    ...
    StockPairedPositionPlannerV3 public immutable positionPlanner;
    ...
    address public immutable quoteAsset0;
    address public immutable quoteAsset1;
    address public immutable quoteAsset2;
    address public immutable quoteAsset3;
    address public immutable quoteAsset4;
    address public immutable quoteAsset5;
    int24 public immutable initialAbsoluteTick0;
    int24 public immutable initialAbsoluteTick1;
    int24 public immutable initialAbsoluteTick2;
    int24 public immutable initialAbsoluteTick3;
    int24 public immutable initialAbsoluteTick4;
    int24 public immutable initialAbsoluteTick5;
    bytes32 public immutable priceConfigurationHash;
    ...
    struct PriceConfiguration {
        address[6] quoteAssets;
        int24[6] initialAbsoluteTicks;
    }
    ...
    error InvalidInitialAbsoluteTick(address quoteAsset, int24 tick);
    ...
    error InvalidQuoteRegistryCoverage(address quoteAsset);
    ...
    error UnsupportedPriceConfiguration(address quoteAsset);
    ...
    constructor(
        ...
        bytes32 expectedPlannerCodeHash = keccak256(type(StockPairedPositionPlannerV3).runtimeCode);
        ...
        for (uint256 index; index < CONFIGURED_QUOTE_ASSET_COUNT; index++) {
            address quoteAsset = priceConfiguration_.quoteAssets[index];
            int24 initialAbsoluteTick = priceConfiguration_.initialAbsoluteTicks[index];
            if (!quoteRegistry_.isSupported(quoteAsset)) revert InvalidQuoteRegistryCoverage(quoteAsset);
            if (
                initialAbsoluteTick <= 0 || initialAbsoluteTick % TICK_SPACING != 0
                    || initialAbsoluteTick >= TickMath.maxUsableTick(TICK_SPACING)
                    || -initialAbsoluteTick <= TickMath.minUsableTick(TICK_SPACING)
            ) {
                revert InvalidInitialAbsoluteTick(quoteAsset, initialAbsoluteTick);
            }
            for (uint256 prior; prior < index; prior++) {
                if (priceConfiguration_.quoteAssets[prior] == quoteAsset) revert DuplicateQuoteAsset(quoteAsset);
            }
        }
        ...
        quoteAsset0 = priceConfiguration_.quoteAssets[0];
        quoteAsset1 = priceConfiguration_.quoteAssets[1];
        quoteAsset2 = priceConfiguration_.quoteAssets[2];
        quoteAsset3 = priceConfiguration_.quoteAssets[3];
        quoteAsset4 = priceConfiguration_.quoteAssets[4];
        quoteAsset5 = priceConfiguration_.quoteAssets[5];
        initialAbsoluteTick0 = priceConfiguration_.initialAbsoluteTicks[0];
        initialAbsoluteTick1 = priceConfiguration_.initialAbsoluteTicks[1];
        initialAbsoluteTick2 = priceConfiguration_.initialAbsoluteTicks[2];
        initialAbsoluteTick3 = priceConfiguration_.initialAbsoluteTicks[3];
        initialAbsoluteTick4 = priceConfiguration_.initialAbsoluteTicks[4];
        initialAbsoluteTick5 = priceConfiguration_.initialAbsoluteTicks[5];
        priceConfigurationHash =
            keccak256(abi.encode(priceConfiguration_.quoteAssets, priceConfiguration_.initialAbsoluteTicks));
    }

    function initialAbsoluteTickFor(address quoteAsset) public view returns (int24) {
        if (quoteAsset == quoteAsset0) return initialAbsoluteTick0;
        if (quoteAsset == quoteAsset1) return initialAbsoluteTick1;
        if (quoteAsset == quoteAsset2) return initialAbsoluteTick2;
        if (quoteAsset == quoteAsset3) return initialAbsoluteTick3;
        if (quoteAsset == quoteAsset4) return initialAbsoluteTick4;
        if (quoteAsset == quoteAsset5) return initialAbsoluteTick5;
        revert UnsupportedPriceConfiguration(quoteAsset);
    }
    ...
    function launch(LaunchParameters calldata parameters) external nonReentrant returns (LaunchResult memory result) {
        ...
        int24 initialAbsoluteTick = initialAbsoluteTickFor(parameters.quoteAsset);
        result.initialTick = result.quoteIsCurrency0 ? initialAbsoluteTick : -initialAbsoluteTick;
        ...
        (plan, position, result.lockedTokenDust) = positionPlanner.buildOneSidedPlan(
            key, result.positionRecipient, result.quoteIsCurrency0, initialAbsoluteTick
        );
    ...
    function _positionSalt(address token, address quoteAsset, address deployer) private pure returns (bytes32) {
        return keccak256(abi.encode("programmable.stock-paired-position.v3", token, quoteAsset, deployer));
    }

    function _rewardVaultSalt(address token, address quoteAsset, address deployer) private pure returns (bytes32) {
        return keccak256(abi.encode("programmable.stock-paired-reward-vault.v3", token, quoteAsset, deployer));
    }
