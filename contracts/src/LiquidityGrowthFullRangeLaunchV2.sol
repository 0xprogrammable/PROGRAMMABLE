...
import { LiquidityGrowthFullRangeAutomationV2 } from "./LiquidityGrowthFullRangeAutomationV2.sol";
import { LiquidityGrowthFullRangePolicyV2 as Policy } from "./LiquidityGrowthFullRangePolicyV2.sol";
import { LiquidityGrowthFullRangePositionPlannerV2 } from "./LiquidityGrowthFullRangePositionPlannerV2.sol";
...
/// @title LiquidityGrowthFullRangeLaunchV2
...
contract LiquidityGrowthFullRangeLaunchV2 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    LiquidityGrowthFullRangeVaultFactoryV2 public immutable growthVaultFactory;
    LiquidityGrowthFullRangeAutomationV2 public immutable automation;
    LiquidityGrowthFullRangePositionPlannerV2 public immutable positionPlanner;
    ...
    event LiquidityGrowthFullRangeTokenLaunchedV2(
    ...
    event LiquidityGrowthFullRangeConfiguredV2(
        address indexed token,
        uint256 totalSupply,
    ...
    event LiquidityGrowthFullRangeCreatorInitialBuyV2(
        address indexed deployer,
        address indexed token,
        bytes32 indexed poolId,
        uint256 nativeAmount,
    ...
    constructor(
        ...
        LiquidityGrowthFullRangeVaultFactoryV2 growthVaultFactory_,
        ...
        (uint256 creatorFee, uint256 programmableFee) = feeHook_.quoteGrossFees(1 ether, TOTAL_SWAP_FEE_BPS);
        if (creatorFee != 0.009 ether || programmableFee != 0.001 ether) {
            revert InvalidSwapFeeQuote(TOTAL_SWAP_FEE_BPS, creatorFee, programmableFee);
        }
        ...
        automation = new LiquidityGrowthFullRangeAutomationV2(growthVaultFactory_, address(this));
        positionPlanner = new LiquidityGrowthFullRangePositionPlannerV2();
        ...
        returns (LaunchResult memory result)
    {
        ...
        result.growthVault = _deployOrReuseGrowthVault(result, key);
        ...
        bytes32 registeredPoolId =
            feeHook.registerPool(key, result.upstreamRewardVault, TOTAL_SWAP_FEE_BPS, TOTAL_SWAP_FEE_BPS);
    ...
    {
        bytes32 salt = keccak256(abi.encode("programmable.full-range.oracle-guard.v2", token, deployer));
        guard = address(
    ...
    function _deployOrReuseGrowthVault(LaunchResult memory result, PoolKey memory key)
        private
        returns (address vaultAddress)
    {
        ...
        bytes32 salt = keccak256(abi.encode("programmable.full-range.vault.v2", result.token, msg.sender));
    ...
    function _deployOrReusePositionRecipient(address token, address deployer) private returns (address recipient) {
        bytes32 salt = keccak256(abi.encode("programmable.full-range.launch-position.v2", token, deployer));
    ...
    function _recordLaunch(
        ...
        _emitLaunchEvents(result, deployer, launchHash);
    }

    function _emitLaunchEvents(LaunchResult memory result, address deployer, bytes32 launchHash) private {
        emit LiquidityGrowthFullRangeTokenLaunchedV2(
            deployer,
            result.token,
            result.poolId,
            address(feeHook),
        ...
        emit LiquidityGrowthFullRangeConfiguredV2(
            result.token,
            TOKEN_SUPPLY,
        ...
        emit LiquidityGrowthFullRangeCreatorInitialBuyV2(
            deployer,
            result.token,
            result.poolId,
