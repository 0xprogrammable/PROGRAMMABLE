...
/// @title ProgrammableCustomPartnerFactoryRegistryV1
/// @notice Append-only approval registry for exact provider-owned Custom factories.
contract ProgrammableCustomPartnerFactoryRegistryV1 is
    AccessControlDefaultAdminRules,
    IProgrammableCustomPartnerFactoryRegistryV1
{
    bytes32 public constant CONFIGURATION_DOMAIN = keccak256("programmable.custom-partner-configuration.v1");
    bytes32 public constant APPROVER_ROLE = keccak256("programmable.custom-partner-factory.approver.v1");
    bytes32 public constant REVOKER_ROLE = keccak256("programmable.custom-partner-factory.revoker.v1");
    ...
    constructor(
        ...
        if (registryGeneration == 0) revert InvalidBinding(bytes32("generation"));
