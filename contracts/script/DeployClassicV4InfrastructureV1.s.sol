...
/// @dev Deploys only four new contracts: HookFactoryV4, the mined HookV4, ClassicPositionPlannerV1 and MemeLaunchV3.
...
contract DeployClassicV4InfrastructureV1 is Script {
    ...
    int24 public constant EXPECTED_LIQUIDITY_TICK_LOWER = 174_800;
    ...
    /// @notice Simulates or, only with Forge `--broadcast`, submits the reviewed four-transaction sequence.
    ...
    function deployReviewed(
        ...
        result.launcher = _deployLauncher(dependencies, inputs, result.feeHook, result.positionPlanner);
        vm.stopBroadcast();
    ...
    function economicsCommitment() public pure returns (bytes32) {
        ...
        bytes32 liquidityCommitment = keccak256(
            abi.encode(
                ...
                int256(EXPECTED_LIQUIDITY_TICK_LOWER),
                uint256(0),
                uint256(1),
                keccak256("canonical-complete-supply-single-permanently-locked-official-v4-position"),
                keccak256("canonical-range-has-a-declared-reachable-liquidity-endpoint")
            )
        );
    ...
    function _validateDeployedStack(
        ...
        _assertSignedValue(
            keccak256("planner.liquidityTickLower"),
            result.positionPlanner.LIQUIDITY_TICK_LOWER(),
            EXPECTED_LIQUIDITY_TICK_LOWER
        );
