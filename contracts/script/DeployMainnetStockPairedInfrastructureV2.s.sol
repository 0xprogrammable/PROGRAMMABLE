...
import { IOndoGMTokenManagerV2, StockQuoteRegistryV2 } from "../src/StockQuoteRegistryV2.sol";

/// @title DeployMainnetStockPairedInfrastructureV2
/// @notice Deterministic, fail-closed deployment path for Stock-Paired V2 on Ethereum Mainnet.
/// @dev A normal Forge script run only simulates. Broadcasting requires an explicit `--broadcast` flag and an
///      operator-controlled signer. The reviewed sequence is exactly seven broadcaster transactions.
contract DeployMainnetStockPairedInfrastructureV2 is Script {
    ...
    address public constant ONDO_GM_TOKEN_MANAGER = 0x2c158BC456e027b2AfFCCadF1BDBD9f5fC4c5C8c;
    ...
    bytes32 public constant ONDO_GM_TOKEN_MANAGER_CODE_HASH =
    ...
    struct DeploymentResult {
        StockQuoteRegistryV2 quoteRegistry;
        ...
        StockPairedEthLaunchCoordinatorV1 ethLaunchCoordinator;
        bytes32 hookSalt;
        bytes32 sourceCommitment;
        uint64 startingNonce;
    }
        ...
        returns (DeploymentResult memory result)
    {
        ...
        result.quoteRegistry = new StockQuoteRegistryV2(
            _assets(),
            _symbolHashes(),
            ONDO_BEACON,
            ONDO_IMPLEMENTATION,
            ...
            ONDO_GM_TOKEN_MANAGER_CODE_HASH
        );
        ...
        result.feeHook = result.hookFactory
            .deploy(
                ...
                configuredTreasury,
                StockQuoteRegistryV1(address(result.quoteRegistry)),
        ...
        result.ethLaunchCoordinator = new StockPairedEthLaunchCoordinatorV1(
            result.launcher,
        ...
        returns (DeploymentPlan memory plan)
    {
        ...
        plan.ethLaunchCoordinator = vm.computeCreateAddress(broadcaster, uint256(startingNonce) + 6);
    ...
    function validateOfficialDependencies() public view {
        ...
        _assertCodeHash(ONDO_GM_TOKEN_MANAGER, ONDO_GM_TOKEN_MANAGER_CODE_HASH);
        _assertAddress(
        ...
        for (uint256 index; index < assets.length; index++) {
            ...
            if (!IOndoGMTokenManagerV2(ONDO_GM_TOKEN_MANAGER).gmTokenAccepted(asset)) {
                revert StockQuoteRegistryV2.TokenNotAcceptedByIssuer(asset);
            }
    ...
    function deploymentSourceCommitment() public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256("programmable.stock-paired.infrastructure.v2.ethereum"),
                _bytecodeCommitment(),
    ...
    function _bytecodeCommitment() private pure returns (bytes32) {
        return keccak256(
            abi.encode(
                keccak256(type(StockQuoteRegistryV2).creationCode),
                ...
                keccak256(type(StockPairedEthLaunchCoordinatorV1).creationCode)
            )
        );
    }
    ...
    function _routingDependencyCommitment() private pure returns (bytes32) {
        return keccak256(abi.encode(_v4RoutingDependencyCommitment(), _v3RoutingDependencyCommitment()));
    }

    function _v4RoutingDependencyCommitment() private pure returns (bytes32) {
        return keccak256(
            abi.encode(
                PERMIT2,
                PERMIT2_CODE_HASH,
    ...
    function _v3RoutingDependencyCommitment() private pure returns (bytes32) {
        return keccak256(
            abi.encode(
                V3_FACTORY,
    ...
    function _validateDeployedStack(DeploymentResult memory result) private view {
        ...
        _assertValue(keccak256("quoteRegistry.assetCount"), result.quoteRegistry.assetCount(), 11);
        ...
        _assertAddress(
            keccak256("quoteRegistry.gmTokenManager"),
            address(result.quoteRegistry.gmTokenManager()),
            ONDO_GM_TOKEN_MANAGER
        );
        ...
        _assertAddress(
            keccak256("ethCoordinator.launcher"),
            address(result.ethLaunchCoordinator.launcher()),
            address(result.launcher)
        );
        _assertAddress(
            keccak256("ethCoordinator.v3SwapRouter"),
            address(result.ethLaunchCoordinator.v3SwapRouter()),
            V3_SWAP_ROUTER
        );
        _assertAddress(
            keccak256("ethCoordinator.v3Factory"), address(result.ethLaunchCoordinator.v3Factory()), V3_FACTORY
        );
        _assertAddress(keccak256("ethCoordinator.weth"), result.ethLaunchCoordinator.weth(), WETH);
        _assertAddress(keccak256("ethCoordinator.usdc"), result.ethLaunchCoordinator.usdc(), USDC);
        address[] memory assets = _assets();
        uint24[] memory fees = _stockPoolFees();
        for (uint256 index; index < assets.length; index++) {
            _assertValue(
                keccak256(abi.encode("ethCoordinator.stockPoolFee", assets[index])),
