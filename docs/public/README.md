...
Robinhood Chain V4 is visible in discovery as planned and not deployed. It is not a public write path, and no live
deployment, trading, fee behavior or indexing claim follows from the published route and schema pointers. Activation
requires separate chain deployment, simulation, wallet, finality, source-verification and indexing evidence.
