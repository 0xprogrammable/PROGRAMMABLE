...
Bids establish an opening price. The standard path auctions 50% of the fixed supply, reserves 50% for liquidity and allocates all auction proceeds to a full-range Uniswap v4 position. It starts about 20 minutes after creation and runs for about four hours on Ethereum. This path does not require the creator to contribute ETH, but it still depends on sufficient demand. Any tokens left after the auction and pool setup return to the creator. The path is enabled only while the pinned CCA factory has no protocol fee controller.
...
The website can describe and save launch plans before contracts are connected. For the tested direct path it derives the raw v4 price, mines the exact hook-address flags and builds the approval or launch call on the server. For the tested auction path it derives the CCA floor, raise threshold, fixed block schedule, convex emission curve and complete LiquidityLauncher migration calldata. Every wallet prompt is gated by current-chain bytecode checks and a read-only simulation. The production deployment manifest remains disabled until:
...
The auction UI intentionally exposes only the economic choice that remains variable in the standard path: the minimum valuation in ETH. The server owns the schedule, allocation, emission curve, v4 fee tier, migration composition, hook and custody parameters.
