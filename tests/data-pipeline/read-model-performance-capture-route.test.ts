...
describe("read-model performance capture route", () => {
  ...
  it("requires a body-bound HMAC and rate-limits the release capture", async () => {
    const releaseBody = {
      ...requestBody,
      schemaVersion: 2,
    ...
    const rawBody = JSON.stringify(releaseBody);
    const signature = `v1=${createHmac("sha256", TOKEN)
      .update(rawBody, "utf8")
      .digest("hex")}`;
    mocks.captureReadModelPerformance.mockResolvedValue({
      schemaVersion: 1,
      captureNonce: releaseBody.captureNonce,
      datasetManifest: { profileId: releaseBody.profileId },
      rpcTrace: { candidateBatchSize: 32 },
    });
    ...
    expect(mocks.captureReadModelPerformance).toHaveBeenCalledWith(releaseBody);
    ...
    expect(replay.headers.get("retry-after")).toBe("30");
  });
