...
describe("Custom Launch API documentation", () => {
  ...
  it("publishes the exact-source and no-broadcast cold-agent path", () => {
    ...
    expect(gitBookGuide).toContain("programmable-launch-3.3.4.tgz");
    expect(rawGuide).toContain("programmable-launch-3.3.4.tgz");
    expect(developerDocsMarkdown).toContain("programmable-launch-3.3.4.tgz");
  ...
  it("keeps the website agent prompt on the current public CLI release", () => {
    expect(createGuide).toContain("public CLI 3.3.4 state machine");
  ...
  it("discloses the exact public V3 fee without conflating LP fees or future operations", () => {
    ...
    for (const source of [gitBookGuide, rawGuide, cliGuide]) {
      expect(source).toContain("programmable-launch-3.3.4.tgz.sha256");
    }
    expect(websiteGuide).toContain("programmable-launch-3.3.4.tgz.sha256");
