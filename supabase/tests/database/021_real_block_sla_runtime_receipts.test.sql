...
create function pg_temp.seed_sla_current_launch_v1()
returns void
language plpgsql
security definer
set search_path = ''
...
select ok(
  ...
  'an exact unpromoted candidate release and stream can be armed once'
);

select pg_catalog.set_config(
  'programmable.test_expired_arm_id',
  programmable_wake_private.arm_real_block_sla_provider_retry_once_v1(
    pg_catalog.repeat('b', 40), 'dpl_22345678901234567890',
    'https://programmable-next.vercel.app', 'prj_programmable_next',
    'stream-expired'
  )::text,
  true
);
...
select programmable_private.register_rpc_provider_deployment(
  ...
  'alchemy', 'real-block-sla-negative-test-v1',
...
select programmable_private.register_rpc_provider_deployment(
  ...
  'quicknode', 'real-block-sla-negative-test-v1',
...
select programmable_private.append_optimistic_market_state_v2(
  ...
  '71000000-0000-4000-8000-000000000001',
  '72000000-0000-4000-8000-000000000002',
  'alchemy-mainnet-31313131313131313131313131313131',
...
select pg_temp.seed_sla_current_launch_v1();
...
select programmable_private.attest_candidate_database_promotion(
  ...
  pg_catalog.repeat('c', 40),
  'dpl_32345678901234567890',
...
select throws_ok(
  ...
  $sql$,
  '55000',
  'SLA provider retry requires unpromoted candidate database',
  'retry arming is rejected after candidate promotion'
);
