-- Projection writers use SQL-safe snake_case operation names while the
-- reviewed bootstrap manifest records semantic kebab-case kinds. Bind the
-- remaining typed fact writers to those exact reviewed kinds.
  ...
  if not exists (
    ...
    where rule.epoch_id = header.epoch_id
      ...
      and (
        ...
        or (p_projection_kind = 'creator_hook_claim' and rule.projection_kind = 'creator-hook-claim')
        or (p_projection_kind = 'launcher_hook_claim' and rule.projection_kind = 'launcher-hook-claim')
        or (p_projection_kind = 'creator_fee_checkpoint' and rule.projection_kind = 'creator-fee-checkpoint')
        or (
          p_projection_kind = 'reward_configuration_activation'
          and rule.projection_kind = 'reward-configuration-activation'
        )
      )
  ) then
    raise exception using errcode = '23514', message = 'event/source role is outside the release writer allowlist';
