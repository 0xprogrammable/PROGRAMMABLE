...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.5.tgz" ${customLaunchCliReleaseUrl}`,
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.5.tgz.sha256" ${customLaunchCliReleaseUrl}.sha256`,
    ...
    "```",
    "Continue only after the checksum reports `OK` and the version is `3.3.5`. The package is `@programmable/launch` and the binary is `programmable-launch`.",
    "",
    ...
    "- `projectMetadata`: canonical `programmable.project-metadata.v1` token identity, presentation and token-target binding for current profile `3.2.0`",
    ...
    "- `launchProfile`: the complete general V3 production profile; current revision 3 uses `programmable.direct-native-hook-graph-profile.v3` and `profileVersion: 3.2.0`; exact `3.1.0` and `3.0.0` remain readable and byte-identical retryable under their original immutable policies",
    ...
    "",
    "Exact profile `3.1.0`, `3.0.0` and revision-2 requests remain readable and byte-identical retryable without project metadata under their original immutable policies. New requests use profile `3.2.0`; V1 creation is permanently write fenced.",
    "",
    ...
    "",
    "`graphBundle` contains exactly `schemaVersion: programmable.custom-graph-bundle.v1`, `sourceBundleSha256`, `targets`, and `pool`. V3 accepts 3 to 16 acyclic direct targets, exactly one `token` target and one `hook` target. Aggregate init code and initializer calldata are limited to 524,288 bytes. For current profile `3.2.0`, `graphBundleHash` is the domain-separated digest of the unbound graph hash plus `projectMetadataHash`; exact legacy requests preserve their original unbound graph hash.",
    "",
    "`projectMetadata` contains the exact token name and symbol, a presentation object with description, nullable image and sorted public links, plus a binding to the primary token target. An image is either explicitly null or backed by exact bytes included in the source manifest. The API preserves these values, and post-deployment `name()` and `symbol()` readback reports `matching`, `mismatch` or `unavailable` without letting a client declare a match.",
    "",
...
export function buildProgrammableLlmsIndex(): string {
  return [
    ...
    `- Custom Launch API guide: ${customLaunchHumanGuideUrl}`,
    `- Custom Launch API readiness: ${customLaunchReadyzUrl}`,
    `- Programmable Launch CLI 3.3.5: ${customLaunchCliReleaseUrl}`,
