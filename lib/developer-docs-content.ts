...
const customLaunchApiGuideUrl =
  "https://programmable.market/docs/developers/custom-launch-api-v1.md";
...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "",
    "`prepared` means an exact artifact exists. `authorized` means the platform permit is attached and the wallet action is ready; it is still not wallet-signed or broadcast. Every response `requestHash` is a labeled `sha256:<64 lowercase hex>` digest. `submitted` means the transaction was observed, not finalized. `finalized` is onchain finality, not Registry publication, website availability, review approval, audit coverage, or a safety claim.",
    "",
    "### Provenance boundary",
    "",
    "A successful Custom Launch API flow can establish that the finalized coin was launched through Programmable when the Router stamp verifies. It does not establish approval, safety, audit coverage, liquidity, tradability, endorsement, or future value.",
    "",
    "## Scope",
    "",
