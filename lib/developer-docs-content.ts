...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "",
    "Private V3 launch history is scoped to the exact authenticated launch principal. A partner root sees only its own launches and does not aggregate child history; each subkey sees only its own. Rotation creates a new launch principal and does not migrate the revoked key's private history. The Router V1 permit-reissue disposition route is wallet-key-only; partner credentials recover by repacking and submitting a new request.",
    "",
    "### Closed request body",
    "",
    ...
    "",
    `Read \`${customLaunchApiOrigin}/v3/custom-launches?limit=10\` with \`custom-launch:read\`. The newest-first collection contains only requests owned by the exact authenticated launch principal: a wallet key's wallet principal, a partner root's own principal or one subkey's own principal. A root does not aggregate child launches and rotation does not migrate private history. \`limit\` defaults to 10 and is capped at 25; pass an unchanged non-null \`nextCursor\` as \`cursor\` for the next page. The collection is durable. For pending rows in the requested page, list reads may perform bounded best-effort chain reconciliation; RPC failure never hides durable history. Use the V3 single-resource route for the full prepared artifact and exact output.`,
    "",
    "### Read launch state",
    "",
