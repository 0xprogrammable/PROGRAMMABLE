...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "",
    `Read \`${customLaunchApiOrigin}/v1/custom-launches?limit=10\` with \`custom-launch:read\`. The newest-first collection contains only requests owned by the key's wallet principal. \`limit\` defaults to 10 and is capped at 25; pass an unchanged non-null \`nextCursor\` as \`cursor\` for the next page. The collection is a durable snapshot and does not perform per-launch chain reconciliation.`,
    "",
    "### Read launch state",
    "",
    ...
    "",
    "After the controller wallet broadcasts the exact transaction, poll the single-resource GET. Reconciliation is request-driven for `authorized` and `submitted` resources. POST only captures the bounded observation window, list reads do not perform per-launch chain reads, and there is no background reconciliation timer. After website discovery refreshes, a finalized launch is eligible to appear in Explore and the connected wallet's Profile without Registry publication. Third-party discovery and listing depend on each indexer implementing the published Router verification flow.",
    "",
