...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    `Robinhood V4 release-candidate OpenAPI: ${customLaunchApiV4OpenApiUrl}`,
    `Robinhood V4 release-candidate pack config: ${customLaunchV4PackConfigSchemaUrl}`,
    ...
    "CLI `3.3.9` is the current public installable release for live Ethereum V3. Robinhood V4 has a deployed Router and backend routes, while package version `4.0.0` remains unpublished and non-installable. This release snapshot keeps `status: release-candidate`, `runtimeStatus: routes-deployed`, `activationStage: pending-public-discovery-promotion`, `publicWrites: false`, `publicAuthorization: false` and `releaseReady: false`.",
    `The stable V4 integration pointers are ${customLaunchApiV4OpenApiUrl}, ${customLaunchV4PackConfigSchemaUrl}, ${customLaunchV4SourceVerificationSchemaUrl} and ${customLaunchV4CapabilitiesUrl}. They describe the deployed release candidate; their presence is not current public activation, write authorization, publication or external-indexing evidence.`,
...
export function buildProgrammableLlmsIndex(): string {
  return [
    "# Programmable",
    "",
    "> Agent guide for live Ethereum V3.3 launch creation, deployed Robinhood V4 release-candidate integration and verified Programmable launch discovery.",
    "",
    ...
    "- Inspect the deployed Robinhood V4 Router and backend release candidate while treating package 4.0.0 as unpublished and non-installable. The installable release remains CLI 3.3.9 for live Ethereum V3.",
    ...
    "- Robinhood V4 public API writes, release readiness, source verification, indexing, trading readiness or publication from deployed runtime, source-candidate 4.0.0 files or stable URLs. This snapshot is `pending-public-discovery-promotion`; live discovery remains authoritative for `publicWrites` and `releaseReady`.",
    ...
    `- Deployed V4 release-candidate resource; public discovery promotion pending: GET ${customLaunchApiOrigin}/v4/chains/4663/custom-launches/{launchId}`,
