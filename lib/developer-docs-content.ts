...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.2.tgz" ${customLaunchCliReleaseUrl}`,
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.2.tgz.sha256" ${customLaunchCliReleaseUrl}.sha256`,
    ...
    "```",
    "Continue only after the checksum reports `OK` and the version is `3.3.2`. The package is `@programmable/launch` and the binary is `programmable-launch`.",
    "",
...
export function buildProgrammableLlmsIndex(): string {
  return [
    ...
    `- Custom Launch API guide: ${customLaunchHumanGuideUrl}`,
    `- Custom Launch API readiness: ${customLaunchReadyzUrl}`,
    `- Programmable Launch CLI 3.3.2: ${customLaunchCliReleaseUrl}`,
