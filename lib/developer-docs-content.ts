...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.1.tgz" ${customLaunchCliReleaseUrl}`,
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.3.1.tgz.sha256" ${customLaunchCliReleaseUrl}.sha256`,
    ...
    "```",
    "Continue only after the checksum reports `OK` and the version is `3.3.1`. The package is `@programmable/launch` and the binary is `programmable-launch`.",
    "",
    ...
    "- `launchProfile`: the complete general V3 production profile; current revision 3 uses `programmable.direct-native-hook-graph-profile.v3` and `profileVersion: 3.1.0`; exact `3.0.0` remains readable and byte-identical retryable",
...
export function buildProgrammableLlmsIndex(): string {
  return [
    ...
    `- Custom Launch API guide: ${customLaunchHumanGuideUrl}`,
    `- Custom Launch API readiness: ${customLaunchReadyzUrl}`,
    `- Programmable Launch CLI 3.3.1: ${customLaunchCliReleaseUrl}`,
