...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "The default profile uses `schemaVersion: programmable.direct-native-hook-graph-profile.v3`, `profileRevision: 3` and `profileVersion: 3.0.0`; its selection binding uses `programmable.direct-native-hook-graph-profile-selection-binding.v3`. Revision 2 remains compatible. The profile accepts a project-owned token, a project-owned hook, every valid Uniswap v4 permission mask and an exact 3-16 target graph.",
    ...
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.2.0.tgz" ${customLaunchCliReleaseUrl}`,
    `curl --fail --location --output "$programmable_cli_dir/programmable-launch-3.2.0.tgz.sha256" ${customLaunchCliReleaseUrl}.sha256`,
    ...
    "```",
    "Continue only after the checksum reports `OK` and the version is `3.2.0`. The package is `@programmable/launch` and the binary is `programmable-launch`.",
    "",
...
export function buildProgrammableLlmsIndex(): string {
  return [
    ...
    `- Custom Launch API guide: ${customLaunchHumanGuideUrl}`,
    `- Custom Launch API readiness: ${customLaunchReadyzUrl}`,
    `- Programmable Launch CLI 3.2.0: ${customLaunchCliReleaseUrl}`,
