...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "### Read launch state",
    "",
    `Read \`${customLaunchApiOrigin}/v3/custom-launches/{launchId}\` with the same Bearer key. The state is one of \`received\`, \`validating\`, \`pending_review\`, \`action_required\`, \`simulating\`, \`prepared\`, \`authorized\`, \`submitted\`, \`finalized\`, \`failed\`, or \`cancelled\`.`,
    "",
