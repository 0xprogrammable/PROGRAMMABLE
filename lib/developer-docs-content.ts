...
export function buildDeveloperDocsMarkdown(): string {
  return [
    ...
    "```",
    "Continue only after the checksum reports `OK` and the version is `3.3.7`. The package is `@programmable/launch` and the binary is `programmable-launch`. Do not substitute the unreleased 3.3.8 source-tree candidate.",
    "",
...
export function buildProgrammableLlmsIndex(): string {
  return [
    ...
    `- Custom Launch API guide: ${customLaunchHumanGuideUrl}`,
    `- Custom Launch API readiness: ${customLaunchReadyzUrl}`,
    `- Live/default Programmable Launch CLI 3.3.7: ${customLaunchCliReleaseUrl}`,
    "- Preparatory only: profile 3.4.0 and CLI 3.3.8 are not released, installable or accepted until backend and .well-known activation",
