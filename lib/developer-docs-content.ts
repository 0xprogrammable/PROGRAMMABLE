...
export function buildDeveloperDocsMarkdown(
  ...
  "",
  `Canonical Custom discovery is **${PROGRAMMABLE_CUSTOM_DISCOVERY_STATUS}** for finalized approved Registry launches. General public submissions remain **${PROGRAMMABLE_CUSTOM_PUBLIC_SUBMISSION_STATUS}**. Resolve the exact generation, contracts, start block and specifications from \`${PROGRAMMABLE_ACTIVE_API_BASE}/manifest\`.`,
  "",
  `The website operations manifest at \`${CUSTOM_REGISTRY_PUBLIC_MANIFEST_PATH}\` separately reports this site's runtime readiness: status \`${registryManifest.status}\`, registry address \`${registryAddress}\`, start block \`${registryStartBlock}\`, ABI URL \`${registryAbiUrl}\` and \`publicSubmissionsEnabled=${registryManifest.publicSubmissionsEnabled}\`. Do not substitute those website-runtime values for the canonical Developer API v2 manifest, and do not subscribe to an unknown address or lookalike event.`,
  "",
  ...
  "",
  `- **Native Custom target policy:** ${PROGRAMMABLE_FEE_POLICY.nativeCustom.totalBps} BPS, or 0.10%, for Programmable. It applies only to the verified official market path. It is not automatically charged on transfers, mints, burns, rewards, games, refunds, bridges or third-party pools. Neither discovery nor intake status proves that a particular fee-bearing path is live.`,
...
export function buildProgrammableLlmsIndex(
  ...
  `- Canonical Custom discovery is \`${PROGRAMMABLE_CUSTOM_DISCOVERY_STATUS}\` for finalized approved Registry launches; general public submissions are \`${PROGRAMMABLE_CUSTOM_PUBLIC_SUBMISSION_STATUS}\`.`,
  `- Resolve canonical address, start block and every binding from the Developer API v${PROGRAMMABLE_ACTIVE_API_VERSION} manifest. \`${CUSTOM_REGISTRY_PUBLIC_MANIFEST_PATH}\` separately reports website runtime readiness.`,
