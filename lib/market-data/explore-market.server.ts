...
export async function readExploreMarketEntriesV1(
  ...
  const fallbackQualifiedCount = fallback.marketRead.qualifiedCount;
  ...
  const observedCount = entries.filter((entry) =>
    snapshots.has(entry.id) || fallbackObservedEntryIds.has(entry.id)
  ).length;
  return {
    ...
    marketRead: {
      ...
      fallbackObservedCount: fallback.marketRead.observedCount,
