...
import { exploreEntryMarketIdentitiesV1 } from "./explore-market-identities";
...
export function isGmgnMarketSnapshotForExploreEntryV1(
  value: unknown,
  entry: ExploreEntry,
): value is GmgnMarketSnapshotV1 {
  if (!isGmgnMarketSnapshotV1(value)) return false;
  return exploreEntryMarketIdentitiesV1(entry).some((identity) =>
    identity.chainId === value.identity.chainId &&
    identity.protocol === value.identity.protocol &&
    identity.tokenAddress === value.identity.tokenAddress &&
    identity.poolId === value.identity.poolId &&
    identity.quoteAddress === value.identity.quoteAddress
  );
}
