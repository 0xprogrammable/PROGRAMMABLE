...
export const PROGRAMMABLE_AGENT_SETUP_TEXT_V1 = Object.freeze([
  ...
  "Submit only the byte-identical current V3.3 request when launchEligibility.deployable is true. The API server remains the decision authority after submission. It must verify the per-launch behavior, fee and liquidity evidence required by the selected lane before exposing any wallet handoff. A needs_evidence or not_executed result is not a pass and cannot be promoted by client, model or attestation output. For unsupported, fix the typed hard-block remediation and rebuild instead of asking for a bypass.",
  ...
  "Use this state machine: pack -> validate --remote -> submit -> server evidence decision -> status --watch --until authorized -> wallet -> status --watch --until finalized. Remote validation first fails closed unless the public profile, revision, version, routes and authentication boundary match this CLI, then sends the same exact V3.3 bytes to non-persisting preflight that consumes no launch-creation quota. Submit only the byte-identical V3.3 request produced by pack and keep the CLI journal and Idempotency-Key unchanged for retries.",
  "Wallet keys require launchWallet to equal their wallet binding. Partner roots and subkeys may select the exact controller in the immutable request but cannot sign for it. A partner root reads every launch attributed to its partner. A subkey reads only its stable lineage, rotation preserves that lineage history, and a separately issued subkey cannot read root or sibling launches. The current Router V1 permit-reissue disposition endpoint accepts wallet keys only; a partner launch recovers by packing and submitting a new request.",
  ...
  "Follow a walletHandoffUrl only after the server-authored resource has verified every behavior, fee and liquidity evidence axis required by the selected lane. If the selected lane is an open arbitrary-custom-hook lane, it carries no automatic Programmable-fee claim. A 10 bps claim exists only for a fee-certified profile or adapter and its exact stamped PoolKey; arbitrary custom hooks are not automatically fee-enforced.",
