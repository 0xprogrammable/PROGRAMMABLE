...
export const ACTIVATION_SCHEMA = "programmable.robinhood-v4-api-activation.v1";
export const ACTIVATION_PATH = "docs/operations/releases/custom-launch-v4/api-activation.json";
export const SUCCESS_EVIDENCE_PATH = "release/robinhood-chain-4663/programmable-launch-v4-clean-room-evidence.json";
export const SUCCESS_ATTESTATION_PATH = "release/robinhood-chain-4663/programmable-launch-v4-clean-room-evidence.attestation.json";
export const CLEAN_ROOM_WORKFLOW = ".github/workflows/programmable-launch-v4-clean-room.yml";
export const REPOSITORY = "programmablehq/PROGRAMMABLE";
...
export const jsonDigest = (value) => `sha256:${createHash("sha256").update(canonicalizeJson(value)).digest("hex")}`;
...
export function assertActivationRecord(record, binding, coordinate) {
  ...
  fail(binding.releaseReady === true && same(binding.blockers, [])
    ...
    && coordinate.repository === REPOSITORY && coordinate.version === "4.0.0"
    && coordinate.tag === "programmable-launch-v4.0.0"
  ...
  fail(evidence?.schemaVersion === "programmable.launch-v4-clean-room-evidence.v1"
  ...
  for (const [field, name] of [
    ["tarballSha256", "programmable-launch-4.0.0.tgz"],
    ["checksumSha256", "programmable-launch-4.0.0.tgz.sha256"],
    ["sbomSha256", "programmable-launch-4.0.0.cdx.json"],
    ["manifestSha256", "programmable-launch-4.0.0.release.json"],
  ...
  fail(evidence.request?.chainId === "4663" && evidence.request.caip2 === "eip155:4663"
    && handoff?.status === "wallet_action_required" && handoff.transactionValueWei === "0"
...
export function projectV4ApiActivation(record, binding, coordinate) {
  let ready = false;
  let invalid = false;
  ...
  return Object.freeze({
    ...
    cliRelease: ready ? { repository: coordinate.repository, tag: coordinate.tag, version: coordinate.version,
      ...
      tarballUrl: `https://github.com/${REPOSITORY}/releases/download/${coordinate.tag}/programmable-launch-4.0.0.tgz`,
      checksumUrl: `https://github.com/${REPOSITORY}/releases/download/${coordinate.tag}/programmable-launch-4.0.0.tgz.sha256`,
    } : null,
