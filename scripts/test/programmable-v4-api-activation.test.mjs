...
function fixture() {
  ...
  const binding = { releaseReady: true, blockers: [], chain: {
  ...
  }, releaseIdentity: { profile: capabilities.profile }, evidence: { chainDeployment: { descriptor: capabilities.chainDeployment } } };
...
function producerMetadata(f) {
  ...
  return { run: {
    ...
    id: Number(f.artifactRef.id), digest: f.artifactRef.digest, name: `programmable-launch-v4-clean-room-evidence-attestation-${producer.runId}-1`, expired: false,
