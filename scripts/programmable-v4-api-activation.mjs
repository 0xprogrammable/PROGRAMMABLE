...
import { validateCleanRoomEvidence, validateReviewedReleaseCoordinate, REVIEWED_RELEASE_COORDINATE_PATH } from "./programmable-launch-v4-clean-room.mjs";
import { V4_RELEASE_BINDING_PATH } from "./programmable-launch-v4-release-binding.mjs";
import { ACTIVATION_SCHEMA, ACTIVATION_PATH, SUCCESS_EVIDENCE_PATH, SUCCESS_ATTESTATION_PATH, CLEAN_ROOM_WORKFLOW, REPOSITORY, assertActivationRecord, bytesDigest, jsonDigest } from "../lib/custom-launch/v4-api-activation.mjs";
...
export function assertVerifiedAttestation(output, evidenceBytes, producer) {
...
export function createActivationRecord(bindingBytes, coordinateBytes, evidenceBytes, bundleBytes, artifact) {
...
function verifyBoundEvidence(root, record, data, evidenceBytes, bundleBytes, verifyArchiveMetadata = false) {
  ...
  for (const relative of [CLEAN_ROOM_WORKFLOW, "scripts/programmable-launch-v4-clean-room.mjs"])
...
export function auditActivation(root) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    const [mode, ...args] = process.argv.slice(2); const options = {};
    ...
    process.stdout.write(`${JSON.stringify(mode === "audit" ? auditActivation(options["repository-root"]) : generate(options))}\n`);
