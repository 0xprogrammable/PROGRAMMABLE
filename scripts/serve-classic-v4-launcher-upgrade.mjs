...
<style>:root{color-scheme:light;--pink:#d279ab;--ink:#241f22;--muted:#746a71;--line:#eadfe5;--wash:#faf5f8;--good:#23745a;--bad:#a93655}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 0,#f9e6f1 0,transparent 34%),#fffdfd;color:var(--ink);font:15px/1.45 ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{width:min(760px,calc(100% - 28px));margin:auto;padding:42px 0}h1{font-size:clamp(32px,7vw,52px);line-height:1;letter-spacing:-.05em;margin:0 0 10px}p{color:var(--muted);margin:0}.card{margin-top:24px;padding:22px;border:1px solid var(--line);border-radius:24px;background:rgba(255,255,255,.9);box-shadow:0 20px 70px rgba(90,35,65,.08)}.facts{display:grid;grid-template-columns:1fr 1fr;gap:10px}.fact{padding:13px;border:1px solid var(--line);border-radius:15px;background:var(--wash)}.fact span{display:block;color:var(--muted);font-size:12px}.fact code{font:12px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.bar{display:flex;flex-wrap:wrap;gap:10px;margin-top:18px}button{appearance:none;border:1px solid var(--line);border-radius:999px;background:#fff;padding:12px 17px;font:inherit;font-weight:650;cursor:pointer}button.primary{background:var(--pink);border-color:var(--pink);color:#fff}button:disabled{opacity:.42;cursor:not-allowed}.notice{margin-top:16px;padding:13px 15px;border-radius:14px;background:var(--wash);color:var(--muted)}.notice.good{background:#eef9f4;color:var(--good)}.notice.bad{background:#fff0f3;color:var(--bad)}.submitted span{display:block;font-size:12px}.submitted a{display:block;color:var(--ink);font:12px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.review{display:none;margin-top:18px;padding-top:18px;border-top:1px solid var(--line)}.review.open{display:block}label{display:flex;gap:10px;align-items:flex-start;margin:14px 0}input{margin-top:4px;accent-color:var(--pink)}footer{margin-top:16px;color:var(--muted);font-size:12px}@media(max-width:620px){.facts{grid-template-columns:1fr}}</style></head>
<body><main><h1>Classic V4 launcher</h1><p>One reviewed Mainnet CREATE transaction. MetaMask remains the only signer.</p><section class="card"><div class="facts"><div class="fact"><span>Required account</span><code>${plan.deployer}</code></div><div class="fact"><span>Created address</span><code>${plan.predictedAddress}</code></div><div class="fact"><span>Nonce</span><strong>${plan.startingNonce}</strong></div><div class="fact"><span>ETH value</span><strong>0 ETH</strong></div><div class="fact"><span>Calldata hash</span><code>${plan.transaction.dataHash}</code></div><div class="fact"><span>Canonical Router</span><code>${plan.router.address}</code></div></div><div class="bar"><button id="network">Switch to Mainnet</button><button id="connect" class="primary">Connect dev wallet</button><button id="prepare" disabled>Run fresh checks</button></div><div id="notice" class="notice">Connect the exact dev wallet to continue.</div><div id="submitted" class="notice submitted" hidden><span>Submitted transaction hash</span><a id="submittedHash" target="_blank" rel="noopener noreferrer"></a></div><div id="review" class="review"><strong>Final owner review</strong><label><input id="ack" type="checkbox"><span>I checked the account, nonce, zero ETH value, created address and calldata hash.</span></label><button id="deploy" class="primary" disabled>Open MetaMask for deployment</button></div></section><footer>No private key is read or stored. This page cannot approve MetaMask; the wallet confirmation remains owner-controlled.</footer></main>
<script>
const config=${configuration};
const $=id=>document.getElementById(id);
const el={network:$("network"),connect:$("connect"),prepare:$("prepare"),notice:$("notice"),submitted:$("submitted"),submittedHash:$("submittedHash"),review:$("review"),ack:$("ack"),deploy:$("deploy")};
const submittedHashStorageKey="programmable.classic-v4-launcher-upgrade.submitted."+config.planDigest;
let provider,account,prepared,busy=false;
function metamask(){if(window.ethereum?.isMetaMask)return window.ethereum;return window.ethereum?.providers?.find(item=>item?.isMetaMask)}
function request(method,params=[]){return provider.request({method,params})}
function notice(text,type=""){el.notice.textContent=text;el.notice.className="notice"+(type?" "+type:"")}
function buttons(){el.connect.disabled=busy;el.network.disabled=busy;el.prepare.disabled=busy||!account;el.deploy.disabled=busy||!prepared||!el.ack.checked}
function showSubmittedHash(hash){
  if(!/^0x[0-9a-f]{64}$/i.test(hash))throw new Error("MetaMask returned an invalid transaction hash");
  el.submittedHash.textContent=hash;
  el.submittedHash.href="https://etherscan.io/tx/"+hash;
  el.submitted.hidden=false;
  try{localStorage.setItem(submittedHashStorageKey,hash)}catch{}
}
function restoreSubmittedHash(){
  try{const hash=localStorage.getItem(submittedHashStorageKey);if(hash)showSubmittedHash(hash)}catch{}
}
async function ensure(){if(String(await request("eth_chainId")).toLowerCase()!==config.chainId)throw new Error("Select Ethereum Mainnet");const accounts=await request("eth_accounts"),selected=String(accounts[0]||"").toLowerCase();if(selected!==config.expectedAccount.toLowerCase())throw new Error("Select the required dev wallet");account=selected}
async function connect(){busy=true;buttons();try{provider=metamask();if(!provider)throw new Error("MetaMask is unavailable");if(!(await request("eth_accounts")).length)await request("eth_requestAccounts");await ensure();notice("Dev wallet connected.","good");el.connect.textContent="Connected"}catch(error){account=undefined;notice(error?.message||String(error),"bad")}finally{busy=false;buttons()}}
async function fresh(){busy=true;prepared=undefined;el.review.classList.remove("open");buttons();try{await ensure();const response=await fetch(config.statePath,{cache:"no-store"}),result=await response.json();if(!response.ok)throw new Error(result.error||"Fresh checks failed");prepared=result.prepared;el.review.classList.add("open");notice("Both RPCs passed. Review once, then open MetaMask.","good")}catch(error){notice(error?.message||String(error),"bad")}finally{busy=false;buttons()}}
async function record(hash){for(let attempt=0;attempt<180;attempt+=1){const response=await fetch(config.recordPath,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({planDigest:config.planDigest,preparedDigest:prepared.preparedDigest,transactionHash:hash})}),result=await response.json();if(response.ok)return result;if(response.status!==409)throw new Error(result.error||"Receipt capture failed");await new Promise(resolve=>setTimeout(resolve,2000))}throw new Error("Transaction is still pending after six minutes")}
async function deploy(){if(busy||!prepared||!el.ack.checked)return;busy=true;buttons();try{await ensure();const response=await fetch(config.statePath,{cache:"no-store"}),freshState=await response.json();if(!response.ok||freshState.prepared?.preparedDigest!==prepared.preparedDigest)throw new Error("Chain state changed. Run fresh checks again");notice("Confirm the exact zero-value CREATE transaction in MetaMask.");const hash=await request("eth_sendTransaction",[prepared.request]);showSubmittedHash(hash);notice("Submitted. Waiting for the receipt.");await record(hash);notice("Launcher receipt captured and verified.","good");prepared=undefined;el.review.classList.remove("open")}catch(error){notice(error?.message||String(error),"bad")}finally{busy=false;buttons()}}
el.network.addEventListener("click",async()=>{provider=metamask();if(!provider)return notice("MetaMask is unavailable","bad");try{await request("wallet_switchEthereumChain",[{chainId:config.chainId}]);notice("Ethereum Mainnet selected.","good")}catch(error){notice(error?.message||String(error),"bad")}});
el.connect.addEventListener("click",connect);
el.prepare.addEventListener("click",fresh);
el.ack.addEventListener("change",buttons);
el.deploy.addEventListener("click",deploy);
restoreSubmittedHash();
buttons();
</script></body></html>`;
}
...
export function isRetryableClassicV4LauncherUpgradeRecordError(error) {
  const message = error?.message ?? String(error);
  return (
    message.includes("not visible on both RPCs") ||
    /HTTP (?:408|429|5\d\d)\b/.test(message) ||
    /(?:timeout|timed out|aborted|fetch failed|ECONNRESET|ETIMEDOUT|ENOTFOUND|socket hang up)/i.test(
      message,
    ) ||
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && url.pathname === `${sessionPath}/record`) {
      try {
        ...
        const message = error?.message ?? String(error);
        const retryable =
          isRetryableClassicV4LauncherUpgradeRecordError(error);
